...
function ensureChatRoom(content, callback) {
	...
	if (window.roomId) {
		...
		// 채팅방 ID가 없으면 /api/chat/create-room API에 targetId를 넘겨 방을 생성 요청
		$.post("/api/chat/create-room", { targetId: window.targetId, productId: productId }, function(res) {
			//서버가 roomId를 응답했을때
			if (res.roomId) {
...
function uploadImage(file, callback) {
	...
	$.ajax({
		url: "/api/chat/upload", // 업로드 첧리 API
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
		success: function(res) {
			callback(res.imageUrl); // 성공 시 S3 URL 반환함
		},
		error: function() {
			showAlert("error", "업로드 실패", "이미지 업로드 중 오류 발생");
		}
	});
}
